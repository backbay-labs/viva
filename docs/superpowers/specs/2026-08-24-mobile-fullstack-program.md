...
| G-2 | Root `bun.lock` omits the mobile workspace → clean-checkout `--frozen-lockfile` fails (P0). Pre-existing scaffold-session dirt (`package.json`, `bun.lock`, `.gitignore`, `bunfig.toml`) was never committed | Root-manifest owner: `bun install` then commit those four files |
| H-1 | `fake_cartesia_gemini` emits 4 PCM bytes (2 samples ≈ 0.08 ms) per `audio_delta` — audible-speech and audible-cancellation gates are unprovable offline by construction. Request: ≥1 s deterministic tone in the fake TTS fixture path | Handoff → adapters lane (remediation Plan 07 owns `cartesia_gemini/runner.rs` + fixtures via Plan 05) |
| H-2 | Immediate reconnect after abrupt loss is rejected `session_cap` for ~45 s (stale-session turn lease; duplicate-session guard). Mobile shows honest copy and recovers post-lease; same-identity takeover or lease release on disconnect would fix the UX | Handoff → agent-service lane |
