...
///
/// `SERVICE-013` puts fixture setup "in tests and development commands", outside
/// `build_study_store`. This is that development command: it lives behind its own
/// entry point, it refuses to run without an explicit development acknowledgement,
/// and the service binary never calls it — so the no-reseed invariant the plan
/// pinned at startup is unchanged.
#[cfg(test)]
